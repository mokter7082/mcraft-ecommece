			...
			<div class="checkoutStep {{ is_object($deliveryInfo) ? 'Complete':'' }}" id="first-step">				
				...
				<div class="checkoutStepContent">
						<div class="area-alert {{ !is_object($deliveryInfo)? 'hide':'' }}">
							...
							<p class="area" id="alarea"> {{ is_object($deliveryInfo) ? $deliveryInfo->area : '' }}</p>
			...
			<form id="checkoutForm" method="POST" action="{{ route('checkout.submit') }}">
				...
				<div class="checkoutStep  {{ is_object($deliveryInfo)? 'activeSlog': 'checkoutSlot' }} " id="secondStep">
							...
							<div class="col-sm-6">
								<section class="day">
									...
									<select name="day_slot" class="c-select" id="daySlot" onchange="DeliveryTime(this)">
										{{-- <?php if($time && ($curh < max(array_keys($time)))): ?> 
										<option value="<?php echo  date("l, d M");?>">Today, <?php echo  date("d M");?></option>
										<?php endif; ?>  --}}
                  ...
                  @endif
							...
							<!-- <div class="col-sm-6">
								<section class="day">
									<label class="select-label">Pick Delivery Time :</label>
									<?php
										if (!empty($time)) {
									?>
										<select name="time_slot" id="timeSlot" class="c-select">
											<?php
											foreach ($time as $tky => $tval) {
												if($curh < max(array_keys($time))){
													if ($curh < $tky) {
														echo '<option value="'.$tval.'">'.$tval.'</option>';
													}													
												}else{
													echo '<option value="'.$tval.'">'.$tval.'</option>';
												}
