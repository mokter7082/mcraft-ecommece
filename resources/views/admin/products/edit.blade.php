        ...
        <div class="box-content">
            ...
            {!! Form::open(['route'=>['admin.edit-products.update', $product->id], 'enctype' => 'multipart/form-data', 'class'=>'form-horizontal']) !!}
                ...
                <fieldset>
                    ...
                    <div class="control-group {{ $errors->has('title') ? 'has-error' : '' }}">
                        {!! Form::label('title', 'Style Name : ', array('class' => 'control-label')) !!}                      
                        <div class="controls">
                          {!! Form::text('sku', $product->sku, ['class'=>'input-xlarge', 'placeholder'=>'Enter Style Name']) !!}
                    ...
                    <div class="control-group {{ $errors->has('stock_qty') ? 'has-error' : '' }}">
                        {!! Form::label('title', 'Stock Quantity :', array('class' => 'control-label')) !!}                      
                        <div class="controls">
                          {!! Form::text('stock_qty', $product->stock_qty, ['class'=>'input-xlarge', 'placeholder'=>'Stock Quantity']) !!}
                    ...
                    <table class="options-table">
                        ...
                        <tbody id="resdata">
                            ...
                            <tr>                                        
                                ...
                                <td><button class="btn btn-danger" data-optid="{{ $option->id }}" onclick="delOption(this); return false;"><i class="halflings-icon trash white"></i> Delete</button></td>
                            </tr>
                            @endforeach  
                        </tbody>
                        <tfoot>
    ...
    <script type="text/javascript">
        ...
        function addOptions(event){
            ...
            if(error) {
                ...
                $.ajax({
                    ...
                    complete: function() {
                        $('#size, #color, #regular_price, #sales_price, #quantity, #images').val('');
        ...
        function delOption(event){
            if (confirm("Are you sure to delete this Option?")) {
                ...
                var optid = $(event).data('optid');
                form_data.append("optid", optid);

                $.ajax({
                    url:"{{ route('admin.deloptions')}}",
                    method:"POST",
                    data:form_data,
                    contentType:false,
                    cache:false,
                    ...
                    success:function(data){
                        ...
                        $.each(result, function(i){
                            output+='<tr><td>'+ result[i].size +'</td><td>'+ result[i].color +'</td><td>'+ result[i].regular_price +'</td><td>'+ result[i].sales_price +'</td><td>'+ result[i].quantity +'</td><td><img src="{{URL::to('images')}}/'+ result[i].images +'" width="70"></td><td><button class="btn btn-danger" data-optid="'+ result[i].id +'" onclick="delOption(this); return false;"><i class="halflings-icon trash white"></i> Delete</button></td></tr>';    
                        });
                    ...
                    complete: function() {
                        $(event).html('<i class="halflings-icon trash white"></i> Delete');
                    }
                })
            }
            return false;
